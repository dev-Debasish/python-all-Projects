...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    # generally, it is used to get the value from the command line and stored in a matrix form.
    columns = []
    for _ in range(cols):
        column = []
    ...
    return columns # it returns the matrix

# generally it prints the transpose of that matrix(up to down). 
...
def get_no_of_lines():
    while True:
        lines = input(
            "Enter the no. of lines to bet on (1-"+ str(MAX_LINES) +")? "
            )
        if lines.isdigit():
            lines = int(lines)
